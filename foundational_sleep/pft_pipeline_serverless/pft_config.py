...
class PFTConfig:
    ...
    def ensure_dirs(self) -> None:
        for d in (self.paths.cache_dir, self.paths.out_dir):
            try:
                d.mkdir(parents=True, exist_ok=True)
            except FileNotFoundError:
                # Volume FUSE may be broken on Serverless GPU.
                # If localize_for_serverless() has already redirected paths
                # to /tmp, this shouldn't happen.  Raise a clear message.
                if str(d).startswith("/Volumes/"):
                    raise RuntimeError(
                        f"Cannot create '{d}' — Volume FUSE is not functional "
                        f"on this compute.\n"
                        f"Fix: call localize_for_serverless(cfg, dbutils) "
                        f"before run(). See volume_compat.py."
                    ) from None
                raise
