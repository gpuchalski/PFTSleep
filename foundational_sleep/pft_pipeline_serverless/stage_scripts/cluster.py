...
def objective(trial):
    ...
    umap_n_neighbors = int(trial.suggest_float("umap_n_neighbors", 15, 100, log=True))
    ...
    hdb_min_cluster_size = trial.suggest_int("hdb_min_cluster_size", 250, 25000, log=True)
    ...
    try:
        ...
        shutil.copyfile(local_csv, "/dbfs/tmp/new_hdbscan_umap.csv")
...
study = optuna.create_study(
    ...
    study_name="umap_hdbscan_trials_new_pipeline",

    storage="sqlite:////local_disk0/umap_hdbscan_trials_new_pipeline.db",
...
_N_TRIALS = int(getattr(cfg.clustering, "optuna_n_trials", 10))
if _RUN_OPTUNA:
...
shutil.copyfile(embedding_path_local, os.path.join(DBFS_OUT, os.path.basename(embedding_path_local)))

shutil.copyfile(labels_path_local, os.path.join(DBFS_OUT, os.path.basename(labels_path_local)))

shutil.copyfile(prob_path_local, os.path.join(DBFS_OUT, os.path.basename(prob_path_local)))
...
def per_cluster_metrics(labels, embedding, probabilities=None,
    ...
    for c in uniq:
        ...
        if persistence is not None and c >= 0 and c < len(persistence):
            # persistence[c] may be a numpy scalar or a length-1 array depending
            # on how cuML's cluster_persistence_ converted; coerce safely.
            pv = np.asarray(persistence[c]).ravel()
            row["persistence"] = float(pv[0]) if pv.size else np.nan
        else:
...
# guarantee a flat 1D float array indexed by cluster id (cuML can return 2D)
if _persist_arr is not None:
    _persist_arr = np.asarray(_persist_arr, dtype=float).ravel()
...
if _persist is not None:
    try:
        ...
        _persist = np.asarray(_persist)
# guarantee flat 1D (cuML can return 2D persistence)
if _persist is not None:
    _persist = np.asarray(_persist, dtype=float).ravel()
