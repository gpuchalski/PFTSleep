...
batch_size = globals().get("batch_size", 1)   # 1 = one full-night recording per
# forward pass. Full sequences (28800s x 125Hz) are huge; batch_size>1 OOMs the
# GPU during attention (a batch of 4 tried to allocate ~9.6 GiB). Keep at 1.
...
# -----------------------------------------------------------------------------
# SERIAL sample_df build + cache.
# The library's calculate_samples_mp uses a multiprocessing.Pool that DEADLOCKS
# on Databricks serverless during pool teardown (the bar reaches 100%, then the
# `with mp.Pool()` exit hangs forever). We avoid it entirely:
#   1. If a cached sample_df parquet exists for this exact config, load it.
#   2. Otherwise build it SERIALLY (plain loop over the library's per-file
#      calculate_samples - no Pool, cannot deadlock), then cache it.
# The dataset constructor accepts sample_df=, which takes its non-MP branch.
# -----------------------------------------------------------------------------
from pftsleep.data_preprocessing import calculate_samples as _calc_samples_one
import pandas as _pd
from tqdm import tqdm as _tqdm

_sdf_cache = cache_dir / (
    f"sample_df__files{len(zarr_files)}__freq{frequency}"
    f"__win{win_length}__hop{hop_length}__max{max_seq_len_sec}.parquet"
)

if _sdf_cache.exists():
    print(f"[extract] loading cached sample_df: {_sdf_cache.name}", flush=True)
    _sample_df = _pd.read_parquet(_sdf_cache)
    print(f"[extract] cached sample_df loaded: {len(_sample_df)} rows", flush=True)
else:
    print(f"[extract] building sample_df SERIALLY over {len(zarr_files)} files "
          "(no multiprocessing pool)...", flush=True)
    _parts = []
    for _i, _zf in enumerate(_tqdm(zarr_files, desc="Building sample_df (serial)",
                                   unit="file")):
        _part = _calc_samples_one(
            _i, str(_zf),
            channels=channels,
            frequency=frequency,
            sample_seq_len_sec=max_seq_len_sec,
            stride_sec=max_seq_len_sec,
            start_offset_sec=0,
            max_seq_len_sec=max_seq_len_sec,
            include_partial_samples=True,
            nan_tolerance=0.0,
        )
        if _part is not None and len(_part):
            _parts.append(_part)
    _sample_df = _pd.concat(_parts, ignore_index=True)
    _sdf_cache.parent.mkdir(parents=True, exist_ok=True)
    _sample_df.to_parquet(_sdf_cache, index=False)
    print(f"[extract] built + cached sample_df: {len(_sample_df)} rows -> "
          f"{_sdf_cache.name}", flush=True)

dataset = SelfSupervisedTimeFrequencyDataset(
    zarr_files=[str(z) for z in zarr_files],  # strings: match sample_df['file'] dtype
    channels=channels,
    frequency=frequency,
    ...
    sample_df=_sample_df,   # <- pass prebuilt df; skips the deadlocking MP path
)
